// =======================================
// pagina_principal.js  (Estado + Rutas OK)
// =======================================

// ---------- Constantes de entorno inyectadas por el HTML ----------
const IS_LOGGED  = typeof window !== "undefined" ? !!window.__LOGGED__ : false;
const USERNAME   = typeof window !== "undefined" ? (window.__USERNAME__ || null) : null;
const API_BASE   = typeof window !== "undefined" ? (window.__API_BASE__ || "/api/productos") : "/api/productos";
...
async function cargarRecomendados() {
  try {
    const data = await fetchJSON(`${API_BASE}/recomendados`);
...
async function showAllProducts(page = 1, perPage = 12) {
  try {
    ...
    const data = await fetchJSON(`${API_BASE}/todos?page=${page}&per_page=${perPage}`);
...
function addToCart(productName, price) {
  if (!IS_LOGGED) {
    ...
    setTimeout(() => { window.location = LOGIN_URL; }, 900);
    return;
  }
...
function getProductImage(_productName) {
  // Puedes mapear por slug/producto real; por ahora placeholder
...
function checkout() {
  if (!IS_LOGGED) {
    ...
    setTimeout(() => { window.location = LOGIN_URL; }, 700);
    return;
  }
...
async function logout() {
  try {
    ...
    if (data && data.redirect) {
      window.location = data.redirect;   // típicamente /login/
    } else {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  setTimeout(() => {
    showAdvancedNotification('🎉 ¡Bienvenido!', IS_LOGGED ? 'Tenemos nuevas ofertas para ti' : 'Explora el catálogo y crea tu cuenta', 'success');
  }, 800);
