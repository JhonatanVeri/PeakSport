...
// ✅ FUNCIÓN GLOBAL PARA CONSTRUIR URLs DE IMÁGENES
function getImageUrl(url) {
  ...
  // Si no tiene el '/' inicial pero tiene 'static/', agregarlo
  if (url.startsWith('static/')) {
    return `/${url}`;
  }
  
  // Caso por defecto: construir la ruta completa
...
function renderTable(items, page, perPage, total) {
  ...
  if (STATE.vistaActual === 'tabla') {
    ...
    tbody.innerHTML = items.map(p => {
      ...
      if (p.imagen_portada) {
        imagenUrl = p.imagen_portada; // URL directa del backend
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center space-x-4">
              <div class="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0">
                ${imagenUrl 
                  ? `<img src="${imagenUrl}" alt="${p.nombre}" class="w-full h-full object-cover" onerror="this.parentElement.innerHTML='<i class=\\'fas fa-image text-gray-400\\'></i>'">`
    ...
    // ✅ VISTA DE TARJETAS - AHORA CON getImageUrl()
      ...
      return `
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden card-hover transition-all">
          <div class="h-48 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center relative overflow-hidden">
            ${imagenUrl 
              ? `<img src="${imagenUrl}" alt="${p.nombre}" class="w-full h-full object-cover" onerror="this.parentElement.innerHTML='<i class=\\'fas fa-box text-gray-400 text-5xl\\'></i>'">`
          ...
          <div class="p-5">
            ...
            <div class="flex items-center gap-2">
              ...
              <button data-id="${p.id}" data-nombre="${(p.nombre || '').replace(/"/g, '&quot;')}" data-slug="${p.slug ?? ''}" data-precio="${precio}" data-imagen_portada ="${p.image}" class="btn-delete flex-1 px-3 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-all text-sm font-medium" title="Eliminar">
                <i class="fas fa-trash"></i>
              </button>
            </div>
          </div>
...
function bindRowActions() {
  ...
  document.querySelectorAll('.btn-delete').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      const imagen_portada = e.currentTarget.getAttribute('data-imagen_portada');
      mostrarModalEliminar(id, nombre, slug, precio,imagen_portada);
    });
  });
}
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .membrete { display: flex; align-items: center; justify-between; border-bottom: 3px solid #000; padding-bottom: 15px; margin-bottom: 20px; }
      ...
      <div class="flex items-center space-x-4">
        <div class="w-16 h-16 bg-gradient-to-br from-red-600 to-black rounded-lg flex items-center justify-center flex-shrink-0">
          <div class="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
            <img src="${imagen_portada || '/static/img/no-image.png'}" 
                alt="${nombre}" 
