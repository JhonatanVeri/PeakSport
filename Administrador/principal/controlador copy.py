...
from flask import (
    render_template, request, jsonify, session, redirect, url_for, current_app, abort
)
...
def _requiere_admin():
    ...
    if rol != "Administrador":
        log_warning("Acceso denegado: requiere rol Administrador")
        abort(403)
...
def registrar_rutas(bp):
    ...
    def api_eliminar_producto(producto_id: int):
        ...
        try:
            ...
            log_info(f"[admin] Producto {producto_id} eliminado correctamente")
    ...
    def api_subir_imagenes(producto_id: int):
        """
        Soporta:
          - multipart con múltiples archivos bajo key: 'files' (o 'imagenes')
          - alt[] opcional (uno por archivo)
          - portada_index opcional (int) -> cuál de los archivos marcar como portada
        """
        prod = obtener_producto_por_id(producto_id)
        if not prod:
            return jsonify({"ok": False, "error": "Producto no encontrado"}), 404
    ...
    # ---------- VISTAS HTML (lista simple de categorías) ----------
    ...
    def vista_categorias():
        try:
            ...
            return render_template("administrador_principal_categorias.html",
    ...
    def api_listar_categorias():
        try:
            ...
            data = [{
                ...
                "descripcion": c.descripcion, "padre_id": c.padre_id
            } for c in cats]
